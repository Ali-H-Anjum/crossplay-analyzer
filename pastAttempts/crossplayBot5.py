...
def get_all_words(word_dictionary, board, tiles, played_moves):
    all_words = set()
    ...
    for played_move in played_moves:
        ...
        for signature, words in word_dictionary.items():
            ...
            for w in words: 
                ...
                if word not in w: #If our word is not in the new word, we can't expand it
                    possible = False
                    break
            ...
            if possible:
                ...
                #word_expansion(board, played_moves, word, words, x, y, direction)
...
def get_moves_in_same_direction(word_dictionary, word, tiles_lower):
    ...
    for signature, potential_expansions in word_dictionary.items():
        ...
        #DOES NOT SKIP words that end up breaking the word on the board
...
def simulated_game_test():
    ...
    #all_moves = get_all_words(word_dictionary, board, player2_tiles, played_moves_to_points.keys())

    get_all_words_improved(word_dictionary, player2_tiles, played_moves_to_points.keys())
