...
def get_all_words(word_dictionary, tiles, played_moves):
    ...
    for played_move in played_moves:
        ...
        same_direction_words = get_words_in_same_direction(word_dictionary, word, x, y, is_descending, tiles_lower)
        different_direction_words = get_words_in_different_direction(word_dictionary, word, x, y, is_descending, tiles_lower)
...
def get_words_in_same_direction(word_dictionary, word, x, y, is_descending, tiles_lower):
    ...
    for signature, potential_expansions in word_dictionary.items():
        ...
        if (is_valid_expansion(signature, available_tile_counts)):
            ...
            for potential_expansion in potential_expansions:
                ...
                print(potential_expansion, potential_expansion_x, potential_expansion_y, is_descending)
                        

    return same_direction_words

def get_words_in_different_direction(word_dictionary, word, x, y, is_descending, tiles_lower):
    ...
    for letter in word:
        ...
        for signature, potential_expansions in word_dictionary.items():
            ...
            if (is_valid_expansion(signature, available_tile_counts)):
                ...
                #Could maybe be reworked so identical to other one
                for potential_expansion in potential_expansions:
                    for i, let in enumerate(word):
                        
                        needed_letters = potential_expansion.replace(let, "", 1)
                        stop = False

                        for needed_letter in needed_letters: #Should be replaced with valid expansion method
                            if(needed_letter not in tiles_lower):
                                stop = True

                            print(needed_letter, tiles_lower, stop)

                        if (stop):
                            continue


                        expansion_difference = potential_expansion.find(let)
                        if(expansion_difference > -1): 
                            if(is_descending):
                                potential_expansion_x = x - expansion_difference
                            ...
                            print(potential_expansion, let, potential_expansion_x, potential_expansion_y, not is_descending)
...
def simulated_game_test():
    ...
    word, x, y, is_descending = move_for_most_points
